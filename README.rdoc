...
See most popular words at the top of the autocompleter::
                         Now the autocompleter sorts words by hit
                         count.
See the ten most popular words in a pane on the index page::
                         The list is updated in real time via AJAX.
...
Dubsar works on the folowing browsers, but with significant styling
glitches:
- Opera 10 on all platforms
- Konqueror 4 on Ubuntu Linux

The {QUnit test suite}[http://dubsar-dictionary.com/qunit/] currently
passes in all listed browsers.
...
Dubsar now tracks the hit count of each word.  A word is considered hit
any time it is rendered into HTML on a page.  The autocompleter now
sorts results by popularity.  Once a decent statistical sample
accumulates, the autocompleter will only display top results and not
attempt to list all matches to potentially enormous searches.
