...
- When a search is submitted (via the _search_ button or a link letter
  ...
  "searching...."  An _info_ icon is displayed, but it does not show up
  due to its color.
- In many scenarios, it is possible to go back to the previous page and
  still see the "searching..." message in the error div, which does not
  fade (fading is only done on page load, which doesn't happen when you
  click back).  This is easily seen if you click one of the letters in
  the header bar; then click another; then click back.
- When a user is viewing a list of results in an accordion, if they
  ...
  accordion will be open after clicking back in this case.  (And as
  mentioned above, the "searching..." message will be displayed in the
  error div.)
- After being successfully redirected to the correct accordion pane
  after an unsuccessful search, the application will not open the
  correct pane if the page is refreshed.
